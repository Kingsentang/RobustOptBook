...
\begin{aligned}
\max &\quad\pmb{c^\prime} \pmb{x}\\\\\\ 
\text {s.t.} &\sum\limits\_{j}{{{\pmb{a}}\_{i j}}{{x}\_{j}}}+\underset{\\\{ \left. {{S}\_{i}}\cup \\\{ {{t}\_{i}} \\\} \right|{{S}\_{i}}\subseteq {{J}\_{i}},\left| {{S}\_{i}} \right|=\left\lfloor {{\Gamma }_{i}} \right\rfloor ,{{t}_{i}}\in {{J}_{i}}\backslash {{S}_{i}} \\\}}{\mathop{\max }}\,\\\{ \sum\limits\_{j\in {{S}\_{i}}}{\hat{\pmb{a}}\_{i j}{{\pmb{y}}\_{j}}+\left( {{\Gamma }\_{i}}-\left\lfloor {{\Gamma }_{i}} \right\rfloor  \right){{\pmb{a}}\_{i{{t}_{i}}}}{{\pmb{y}}\_{{{t}\_{i}}}}} \\\}\le {{\pmb{b}}_{i}}&&\forall i \\\\\\ 
&{\pmb{1} \leq \pmb{x} \leq \pmb{u}} \\\\\\ 
&{-\pmb{y}\_{j} \leq x\_{j} \leq \pmb{y}\_{j}} && \forall j \\\\\\ 
&{\pmb{y} \geq 0}
\end{aligned}
