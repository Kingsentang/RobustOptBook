...
对比公式$\eqref{Soyester}$和公式$\eqref{Soyester counterpart}$，主要区别在于第一条约束，推导如下：
...
令$y=|x|$即可由公式$\eqref{Soyester}$推导至公式$\eqref{Soyester counterpart}$。
...
对于模型$\eqref{Soyester}$，Ben-tal和Nemirovski提出了不同的求解思路，鲁棒对应模型如下：
$$
\begin{align}
...
$$
\begin{align}
\max &\quad\pmb{c^\prime} \pmb{x} \label{BertSim}\\\\\\ 
...
$$
\begin{align}
\max &\sum_{i=1}^{n}p_{i}x_{i}-\phi \label{example} \sum_{i=1}^{n}\sigma_{i}^{2}x_{i}^{2}\notag\\\\\\
