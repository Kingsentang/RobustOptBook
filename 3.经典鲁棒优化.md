...
$$
\begin{align}
\max &\quad\pmb{c^\prime} \pmb{x} + \mathop {\max }\limits_{\\\{ {\left. {{S_0}} \right|{S_0} \subseteq {J_0},\left| {{S_0}} \right| \leq {\Gamma_0}} \\\}} \\\{ {\sum\limits\_{j \in {S_0}} {{{\pmb{d}}\_j}\left| {{{x}\_j}} \right|} } \\\}\\\\\\ 
...
其中，$\Gamma_{0}$是区间$\left[0, | J\_{0} \|\right.$内的整数，$J\_{0}=\\\{j | d\_{j}>0\\\}$ $\left|J_{0}\right|$，是$\pmb{c}$中不确定元素的个数。Bertsimas和Sim考虑了目标函数的不确定性，所以有了（3.17）中目标函数的表达形式。对于目标函数中的$\pmb{c}$，允许变化的不确定元素有$\Gamma\_{0}$个，取值为$\mathop {\pmb{c^\top}}\limits\_{j \in {S_0}}  + \mathop {\max }\limits\_{\\\{ {\left. {{S_0}} \right|{S_0} \subseteq {J_0},\left| {{S_0}} \right| \leq {\Gamma \_0}} \\\}} \sum\limits_{j \in {S_0}} {{\pmb{d}\_j}} $，其余均为标称值。
