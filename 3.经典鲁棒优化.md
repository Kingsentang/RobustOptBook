...
$$
\begin{array}{rl}
\max \min\_{z \in U} &\sum\_{i=1}^{n}\left(p\_{i}-\sigma\_{i} z\_{i}\right) x\_{i} \\\\\\
\text{ s.t. } &\sum\_{i=1}^{n} x\_{i}=1 \\\\\\
&x\_{i} \geq 0
\end{array}
$$
其中，$z=\phi \sigma x$,$\sigma\_{i} z\_{i}$可以理解为$p_i$的偏移量，即模型（3.15）中的$\hat{\mathbf{a}}\_{ij}$。因此，模型（3.20）类似模型（3.2)，套用Bertsimas和Sim鲁棒对等模型（3.15）可得到：
$$
\begin{align}
\max &\quad z\\\\\\ 
