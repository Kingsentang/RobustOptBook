...
```{example} 单阶段库存管理模型 \label{example::single period inventory management}
...
```{theorem}\label{theorem::upper bound}
	$Z^{*} \leq Z^L$.
```
...
& \alpha_s + \displaystyle \sum_{k \in \mathcal{K}\_s} \pmb{\beta}\_k^\prime\pmb{z}  \geq \pmb{r}^\prime(s)\pmb{G}\_m(s)\pmb{z} + h\_m(s) & \forall \pmb{z} \in \mathcal{Z}\_s,  s \in [S] \\\\\\
